...
def _service_environment_values() -> dict[str, str]:
...
def _set_service_manager_environment() -> None:
    values = _service_environment_values()
    if not values:
        return
    _run(["systemctl", "--user", "set-environment", *[f"{key}={value}" for key, value in sorted(values.items())]])
...
def _base_env_lines() -> list[str]:
    ...
    env_lines.append("PassEnvironment=SHERPADESK_API_KEY SHERPADESK_API_USER SHERPAMIND_OPENCLAW_WEBHOOK_URL SHERPAMIND_OPENCLAW_WEBHOOK_TOKEN")
    return env_lines
