...
_ENTITY_METADATA_FIELDS: list[tuple[str, str]] = [
    ...
    ("attachment_names_csv", "attachment_names"),
    ("attachment_name_tokens_csv", "attachment_name_tokens"),
...
def _entity_retrieval_health(conn, where_clause: str, params: tuple[object, ...]) -> dict:
    row = conn.execute(
        ...
        SELECT COUNT(*) AS total_tickets,
               ...
               SUM(CASE WHEN json_extract(document_raw_json, '$.metadata.attachment_names_csv') IS NOT NULL AND json_extract(document_raw_json, '$.metadata.attachment_names_csv') != '' THEN 1 ELSE 0 END) AS attachment_names_tickets,
               SUM(CASE WHEN json_extract(document_raw_json, '$.metadata.attachment_name_tokens_csv') IS NOT NULL AND json_extract(document_raw_json, '$.metadata.attachment_name_tokens_csv') != '' THEN 1 ELSE 0 END) AS attachment_name_tokens_tickets,
