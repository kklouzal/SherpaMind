...
def _build_hook_payload(settings: Settings, ticket_id: str, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "Use the SherpaMind skill/query surface to research whether a genuinely similar prior issue has been seen before, and use that evidence to strengthen the 'First checks / next steps' bullets when confidence is high. "
        "Do not just mention past tickets abstractly — if you find a strong match, fold the learned pattern into the recommended next steps in a concrete way. "
        "Only claim 'seen before' when confidence is high based on strong retrieval evidence; if confidence is weak or ambiguous, say no high-confidence match was found instead of bluffing. "
...
def _build_ticket_update_payload(settings: Settings, ticket_id: str, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "Use the SherpaMind skill/query surface to research whether a genuinely similar prior issue has been seen before, and use that evidence to strengthen the recommended follow-up / next steps when confidence is high. "
        "Do not just mention past tickets abstractly — if you find a strong match, fold the learned pattern into the next-step bullets in a concrete way. "
        "Only claim 'seen before' when confidence is high based on strong retrieval evidence; if confidence is weak or ambiguous, omit the match claim or state that no high-confidence match was found. "
