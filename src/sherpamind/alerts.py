...
def _build_hook_payload(settings: Settings, ticket_id: str, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "Use the SherpaMind skill/query surface to research whether a genuinely similar prior issue has been seen before, and use that to strengthen the 'First checks / next steps' recommendation. "
        "Only claim 'seen before' when confidence is high based on strong retrieval evidence; if confidence is weak or ambiguous, omit it instead of bluffing. "
...
def _build_ticket_update_payload(settings: Settings, ticket_id: str, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "Use the SherpaMind skill/query surface to research whether a genuinely similar prior issue has been seen before, and use that to strengthen the recommended follow-up / next steps. "
