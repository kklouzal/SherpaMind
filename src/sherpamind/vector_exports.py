...
def _load_rows(db_path: Path, limit: int | None = None) -> list[dict[str, Any]]:
    query = """
        SELECT c.chunk_id,
               ...
               json_extract(d.raw_json, '$.metadata.attachment_names_csv') AS attachment_names_csv,
               json_extract(d.raw_json, '$.metadata.attachment_name_tokens_csv') AS attachment_name_tokens_csv,
               json_extract(d.raw_json, '$.metadata.attachment_name_token_count') AS attachment_name_token_count,
               ...
               json_extract(d.raw_json, '$.metadata.cleaned_latest_public_note') AS cleaned_latest_public_note,
               json_extract(d.raw_json, '$.metadata.latest_public_note_date') AS latest_public_note_date,
               json_extract(d.raw_json, '$.metadata.cleaned_latest_internal_note') AS cleaned_latest_internal_note,
               json_extract(d.raw_json, '$.metadata.latest_internal_note_date') AS latest_internal_note_date,
        ...
        for record in rows:
            ...
            payload = {
                ...
                "metadata": {
                    ...
                    "attachment_names": record["attachment_names_csv"],
                    "attachment_name_tokens": record["attachment_name_tokens_csv"],
                    "attachment_name_token_count": record["attachment_name_token_count"],
                    ...
                    "cleaned_latest_public_note": record["cleaned_latest_public_note"],
                    "latest_public_note_date": record["latest_public_note_date"],
                    "cleaned_latest_internal_note": record["cleaned_latest_internal_note"],
                    "latest_internal_note_date": record["latest_internal_note_date"],
...
CHUNK_SECTION_PREFIXES: dict[str, tuple[str, ...]] = {
    ...
    "activity": (
        ...
        "Latest public note date:",
        "Latest public note:",
        "Latest internal note date:",
    ...
    def _has_activity_context(doc: dict[str, Any]) -> bool:
        return any(
            ...
            for field in (
                ...
                "cleaned_latest_public_note",
                "cleaned_latest_internal_note",
...
def get_retrieval_readiness_summary(db_path: Path, limit: int | None = None) -> dict[str, Any]:
    ...
    metadata_fields = {
        ...
        "cleaned_latest_public_note": lambda row: _present(row.get("cleaned_latest_public_note")),
        "latest_public_note_date": lambda row: _present(row.get("latest_public_note_date")),
        "cleaned_latest_internal_note": lambda row: _present(row.get("cleaned_latest_internal_note")),
        "latest_internal_note_date": lambda row: _present(row.get("latest_internal_note_date")),
        ...
        "attachment_names": lambda row: _present(row.get("attachment_names_csv")),
        "attachment_name_tokens": lambda row: _present(row.get("attachment_name_tokens_csv")),
        "attachment_name_token_count": lambda row: int(row.get("attachment_name_token_count") or 0) > 0,
