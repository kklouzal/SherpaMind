...
def _retrieval_signal_pressure_rows(summary: dict[str, Any], dimension: str) -> list[dict[str, Any]]:
    ...
    for row in ((summary.get("retrieval_signal_pressure") or {}).get(dimension) or {}).get("rows", []):
        rows.append({
            ...
            "issue_ratio": _format_ratio(row.get("issue_ratio")),
...
def _detail_gap_rows(coverage: dict[str, Any], dimension: str) -> list[dict[str, Any]]:
    ...
    for row in ((coverage.get("detail_gap_pressure") or {}).get(dimension) or {}).get("rows", []):
        rows.append({
            ...
            "actionable_backlog": row.get("actionable_backlog", 0),
            "permanent_failure_backlog": row.get("permanent_failure_backlog", 0),
            "cooling_down_backlog": row.get("cooling_down_backlog", 0),
            "tracked_failure_backlog": row.get("tracked_failure_backlog", 0),
...
def generate_public_snapshot(db_path: Path) -> dict:
    ...
    snapshot_md = [
        ...
        _markdown_table(_detail_gap_rows(enrichment_coverage, "accounts"), [
            ...
            ("permanent_failure_backlog", "Permanent-Failure Backlog"),
            ("cooling_down_backlog", "Cooling-Down Backlog"),
            ("tracked_failure_backlog", "Tracked-Failure Backlog"),
        ...
        "",
        "## Enrichment pressure — under-covered departments",
        "",
        _markdown_table(_detail_gap_rows(enrichment_coverage, "departments"), [
            ("label", "Department"),
            ("tickets", "Tickets"),
            ("detail_tickets", "Detail Tickets"),
            ("detail_ratio", "Detail Coverage"),
    ...
    retrieval_md = [
        ...
        "",
        "## Retrieval signal pressure — under-covered departments",
        "",
        _markdown_table(
            _retrieval_signal_pressure_rows(retrieval_readiness, "departments"),
            [
                ("label", "Department"),
                ("documents", "Docs"),
                ("richness_ratio", "Richness"),
