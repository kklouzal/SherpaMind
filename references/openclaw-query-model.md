...
- `python3 scripts/run.py report-enrichment-coverage` (now includes detail-gap pressure across under-covered accounts/categories/departments/technicians plus actionable-vs-blocked backlog splits so enrichment breadth can be inspected directly)
...
- whether a limited/sampled readiness run is being reported honestly against its sampled ticket scope instead of mixing sample-sized materialization counts with full-corpus source counts
...
- which accounts/categories/departments/technicians are still metadata-poor across key retrieval signals (detail, issue/action/activity/resolution/attachment context) so enrichment work can target the thinnest high-value slices first
