...
4. carry stable workflow/state metadata forward with those documents (subject cleanup, user/account/technician linkage, human-readable entity labels, label-source provenance, normalized priority/category/class/submission/resolution taxonomy labels, human-readable department labels, user/creator/technician email domains, participant email-domain rollups, recent log types, log-interaction counts/date summaries, distinct public/internal participant counts, latest/recent participant labels, latest public/internal note text plus note dates, mixed-visibility activity flags, next-step hints, derived action-cue text/source, explicit-followup-vs-waiting-log cue provenance, request-completion/follow-up cues, log-derived latest-response and closure cues, contract/confirmation context, account-location and department context, intake-channel/handling flags, project/scheduled-ticket linkage, effort-tracking signals, attachment presence plus normalized attachment name/name-token/extension/kind/size summaries (including size-known counts and broader kind-family counts), resolution highlights)
...
6. expose whether remaining detail backlog is still actionable versus currently blocked by tracked permanent failures or temporary cooldown so enrichment steering can target real opportunities instead of repeatedly chasing known-bad slices
7. chunk long text where needed, rebalancing tiny leading/trailing leftovers when possible so vector rows stay retrieval-meaningful instead of collapsing into metadata-only fragments
8. index documents into:
   - FTS/keyword search
   - vector embeddings/index
9. expose hybrid query commands for OpenClaw/tooling
...
Retrieval observability should also expose **retrieval signal pressure** by account/category/department/technician so the backend can see where key prepared-signal families (detail, issue context, action cues, activity history, resolution context, attachment context) are still sparse instead of only knowing global coverage averages. Detail-signal coverage should key off actual `ticket_details` row presence (the real enrichment boundary), not merely the existence of baseline ticket fields that happen to be useful for retrieval.
